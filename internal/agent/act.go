...
func (e *AgentEngine) executeToolCalls(
	...
	if e.config.ParallelToolCalls && n >= 2 {
		e.executeToolCallsParallel(ctx, response, step, state, iteration, sessionID, assistantMessageID)
		return
	}

	for i, tc := range response.ToolCalls {
		e.executeSingleToolCall(ctx, tc, i, step, state, iteration, round, sessionID, assistantMessageID)
	}
}
...
func (e *AgentEngine) executeToolCallsParallel(
	...
	for _, toolCall := range results {
		...
		e.emitToolKnowledgeReferences(ctx, state, toolCall, result, iteration, sessionID)
...
func (e *AgentEngine) emitToolKnowledgeReferences(
	ctx context.Context,
	...
	if state == nil || result == nil || !result.Success {
		return
	}
	refs := extractKnowledgeReferencesFromToolResult(toolCall.Name, result)
	if len(refs) == 0 {
		return
	}
	added := appendUniqueKnowledgeReferences(state, refs)
	if len(added) == 0 {
		return
	}
	e.eventBus.Emit(ctx, event.Event{
...
func extractKnowledgeReferencesFromToolResult(toolName string, result *types.ToolResult) []*types.SearchResult {
...
func searchReferencesFromData(data map[string]interface{}) []*types.SearchResult {
	var refs []*types.SearchResult
	for _, item := range mapItems(data["results"]) {
		if ref := searchResultFromMap(item); ref != nil {
...
func wikiReferencesFromData(data map[string]interface{}) []*types.SearchResult {
	...
	for _, item := range mapItems(data["wiki_pages"]) {
		if ref := wikiPageReferenceFromMap(item); ref != nil {
			refs = append(refs, ref)
		}
	}
	return refs
...
func appendUniqueKnowledgeReferences(state *types.AgentState, refs []*types.SearchResult) []*types.SearchResult {
	...
	for _, ref := range state.KnowledgeRefs {
		if key := knowledgeReferenceKey(ref); key != "" {
			seen[key] = true
		}
	}
	added := make([]*types.SearchResult, 0, len(refs))
	for _, ref := range refs {
		key := knowledgeReferenceKey(ref)
		if key == "" || seen[key] {
			continue
		}
		seen[key] = true
...
func knowledgeReferenceKey(ref *types.SearchResult) string {
	if ref == nil || ref.ID == "" {
		return ""
	}
	if ref.KnowledgeID != "" {
		return ref.KnowledgeID + ":" + ref.ID
	}
	return ref.ID
}

func searchResultFromMap(item map[string]interface{}) *types.SearchResult {
	chunkID := stringFromMap(item, "chunk_id", "id")
	knowledgeID := stringFromMap(item, "knowledge_id", "external_doc_id")
	if chunkID == "" || knowledgeID == "" {
		return nil
	}
	return &types.SearchResult{
		ID:                chunkID,
		Content:           stringFromMap(item, "content", "text", "matched_content"),
		KnowledgeID:       knowledgeID,
		KnowledgeTitle:    stringFromMap(item, "knowledge_title", "title", "knowledge_filename"),
		KnowledgeBaseID:   stringFromMap(item, "knowledge_base_id"),
		ChunkIndex:        intFromMap(item, "chunk_index"),
...
func wikiPageReferenceFromMap(item map[string]interface{}) *types.SearchResult {
	slug := stringFromMap(item, "wiki_page_slug", "slug")
	wikiPageID := stringFromMap(item, "wiki_page_id", "page_id", "id")
	if wikiPageID == "" {
		wikiPageID = slug
	}
	if wikiPageID == "" {
	...
	title := stringFromMap(item, "title", "wiki_title", "page_title")
	if title == "" {
		title = slug
	}
	content := stringFromMap(item, "summary", "content", "text")
	if content == "" {
		content = title
	}
	...
	if slug != "" {
		metadata["weknora_slug"] = slug
		metadata["weknora_wiki_page_slug"] = slug
		metadata["wiki_page_slug"] = slug
	}
	if pageType := stringFromMap(item, "page_type"); pageType != "" {
		metadata["weknora_page_type"] = pageType
	}
	return &types.SearchResult{
		ID:              wikiPageID,
		Content:         content,
...
func stringFromMap(item map[string]interface{}, keys ...string) string {
...
func intFromMap(item map[string]interface{}, key string) int {
	switch value := item[key].(type) {
	case int:
		return value
	case int64:
...
func floatFromMap(item map[string]interface{}, key string) float64 {
	switch value := item[key].(type) {
	case float64:
		return value
	case float32:
