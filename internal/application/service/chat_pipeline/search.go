...
func (p *PluginSearch) OnEvent(ctx context.Context,
	...
	chatManage.SearchResult = filterToExplicitKnowledgeScope(ctx, chatManage, allResults)
	...
	if chatManage.EnableQueryExpansion && len(chatManage.SearchResult) < max(1, chatManage.EmbeddingTopK) {
		...
		if len(expResults) > 0 {
			...
			chatManage.SearchResult = filterToExplicitKnowledgeScope(ctx, chatManage, chatManage.SearchResult)
		}
	}
...
func filterToExplicitKnowledgeScope(
	ctx context.Context,
	chatManage *types.ChatManage,
	results []*types.SearchResult,
) []*types.SearchResult {
	scope := explicitKnowledgeScope(chatManage)
	if len(scope) == 0 || len(results) == 0 {
		return results
	}
	...
	for _, result := range results {
		...
		if _, ok := scope[result.KnowledgeID]; ok {
			filtered = append(filtered, result)
		}
	}
	if dropped := len(results) - len(filtered); dropped > 0 {
		pipelineWarn(ctx, "Search", "scope_filter_drop", map[string]interface{}{
			"requested_knowledge_ids": len(scope),
...
func explicitKnowledgeScope(chatManage *types.ChatManage) map[string]struct{} {
	if chatManage == nil || len(chatManage.KnowledgeIDs) == 0 {
		return nil
	}
	...
	if len(chatManage.SearchTargets) == 0 && len(chatManage.KnowledgeBaseIDs) > 0 {
		return nil
	}
	scope := make(map[string]struct{}, len(chatManage.KnowledgeIDs))
	for _, knowledgeID := range chatManage.KnowledgeIDs {
		knowledgeID = strings.TrimSpace(knowledgeID)
		if knowledgeID == "" {
			continue
		}
		scope[knowledgeID] = struct{}{}
	}
	return scope
}
