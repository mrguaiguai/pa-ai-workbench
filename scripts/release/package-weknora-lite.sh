...
#
# 用法:
#   ./scripts/release/package-weknora-lite.sh              # 自动检测版本
#   ./scripts/release/package-weknora-lite.sh v0.2.0       # 指定版本号
#   SKIP_FRONTEND=1 ./scripts/release/package-weknora-lite.sh  # 跳过前端构建（使用已有 web/）
#

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
REPOSITORY_ROOT="$(cd "${SCRIPT_DIR}/../.." && pwd)"
ROOT_DIR="$REPOSITORY_ROOT/platform/weknora"
cd "${ROOT_DIR}"
...
export EDITION=lite
eval "$("$REPOSITORY_ROOT/scripts/release/weknora-version.sh" env)"
LDFLAGS="-w -s $("$REPOSITORY_ROOT/scripts/release/weknora-version.sh" ldflags)"
