...
"$REPOSITORY_ROOT/scripts/dev/weknora-dev.sh" start
if [ $? -ne 0 ]; then
    log_error "基础设施启动失败"
    exit 1
fi
...
if [ "$start_backend" = "y" ] || [ "$start_backend" = "Y" ]; then
    log_info "启动后端..."
    # 在后台启动后端
    nohup bash -c 'cd "'$PROJECT_ROOT'" && "$REPOSITORY_ROOT/scripts/dev/weknora-dev.sh" app' > "$PROJECT_ROOT/logs/backend.log" 2>&1 &
    BACKEND_PID=$!
    ...
    log_warning "跳过后端启动"
    log_info "稍后在新终端运行: make dev-app 或 "$REPOSITORY_ROOT/scripts/dev/weknora-dev.sh" app"
fi
...
if [ "$start_frontend" = "y" ] || [ "$start_frontend" = "Y" ]; then
    ...
    log_warning "跳过前端启动"
    log_info "稍后在新终端运行: make dev-frontend 或 "$REPOSITORY_ROOT/scripts/dev/weknora-dev.sh" frontend"
fi
