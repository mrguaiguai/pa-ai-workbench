...
CANONICAL_DEV_RUNTIME_DIR="$LOCAL_RUNTIME_ROOT/pa-dev"
...
if legacy_backend_runtime_present \
  || [[ -d "$LEGACY_PA_RUNTIME_ROOT/tmp/dev-services" \
    || -d "$LEGACY_PA_RUNTIME_ROOT/logs/dev-services" ]]; then
  BACKEND_RUNTIME_DIR="$LEGACY_BACKEND_RUNTIME_DIR"
  RUNTIME_DIR="$LEGACY_PA_RUNTIME_ROOT/tmp/dev-services"
  LOG_DIR="$LEGACY_PA_RUNTIME_ROOT/logs/dev-services"
else
  BACKEND_RUNTIME_DIR="$CANONICAL_BACKEND_RUNTIME_DIR"
  RUNTIME_DIR="$CANONICAL_DEV_RUNTIME_DIR/run"
  LOG_DIR="$CANONICAL_DEV_RUNTIME_DIR/logs"
fi
...
start_backend() {
  ...
  (
    cd "$BACKEND_DIR"
    PYTHONPATH="$PYTHONPATH_VALUE" PA_BACKEND_RUNTIME_DIR="$BACKEND_RUNTIME_DIR" \
