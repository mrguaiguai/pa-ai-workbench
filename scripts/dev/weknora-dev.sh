...
start_services() {
    ...
    # 启动服务
    "$DOCKER_COMPOSE_BIN" $DOCKER_COMPOSE_SUBCMD -f "$REPOSITORY_ROOT/infra/compose/weknora.dev.yaml" $PROFILES up -d
...
stop_services() {
    ...
    cd "$PROJECT_ROOT"
    "$DOCKER_COMPOSE_BIN" $DOCKER_COMPOSE_SUBCMD -f "$REPOSITORY_ROOT/infra/compose/weknora.dev.yaml" down
...
show_logs() {
    cd "$PROJECT_ROOT"
    "$DOCKER_COMPOSE_BIN" $DOCKER_COMPOSE_SUBCMD -f "$REPOSITORY_ROOT/infra/compose/weknora.dev.yaml" logs -f
}

# 查看状态
show_status() {
    cd "$PROJECT_ROOT"
    "$DOCKER_COMPOSE_BIN" $DOCKER_COMPOSE_SUBCMD -f "$REPOSITORY_ROOT/infra/compose/weknora.dev.yaml" ps
}
...
start_app() {
    ...
    if command -v air &> /dev/null; then
        ...
        LDFLAGS="$("$REPOSITORY_ROOT/scripts/release/weknora-version.sh" ldflags) -X 'google.golang.org/protobuf/reflect/protoregistry.conflictPolicy=warn'"
