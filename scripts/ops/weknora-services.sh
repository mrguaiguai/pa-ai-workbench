...
PROJECT_ROOT="$REPOSITORY_ROOT/platform/weknora"
COMPOSE_FILE="$REPOSITORY_ROOT/infra/compose/weknora.yaml"
...
compose() {
    "$DOCKER_COMPOSE_BIN" $DOCKER_COMPOSE_SUBCMD -f "$COMPOSE_FILE" "$@"
}
...
ensure_sandbox_image() {
    ...
    # 后台拉取，不阻塞主流程
    (
        if PLATFORM=$PLATFORM compose --profile sandbox pull sandbox 2>/dev/null; then
            log_success "沙箱镜像拉取完成: $sandbox_image"
        else
            log_warning "沙箱镜像拉取失败，Agent Skills 功能可能不可用"
            log_warning "可稍后通过 scripts/ops/weknora-services.sh --pull 拉取"
        fi
    ) &
...
start_docker() {
	...
	if [ "$NO_PULL" = true ]; then
		...
		PLATFORM=$PLATFORM compose up --build -d
	else
		# 拉取最新镜像
		log_info "拉取最新镜像..."
		PLATFORM=$PLATFORM compose up --pull always -d
	fi
    if [ $? -ne 0 ]; then
...
pull_images() {
	...
	PLATFORM=$PLATFORM compose pull
    if [ $? -ne 0 ]; then
        log_error "镜像拉取失败"
        return 1
    fi
    ...
    log_info "拉取沙箱镜像..."
    PLATFORM=$PLATFORM compose --profile sandbox pull sandbox 2>/dev/null || \
...
restart_container() {
    ...
    # 检查容器是否存在
	if ! compose ps --services | grep -q "^$container_name$"; then
	...
	PLATFORM=$PLATFORM compose build "$container_name"
    if [ $? -ne 0 ]; then
	...
	PLATFORM=$PLATFORM compose up -d --no-deps "$container_name"
    if [ $? -ne 0 ]; then
...
if [ "$STOP_SERVICES" = true ]; then
    ...
    if [ "$START_OLLAMA" = true ] && [ "$START_DOCKER" = true ]; then
        if [ $OLLAMA_RESULT -eq 0 ] && [ $DOCKER_RESULT -eq 0 ]; then
            ...
            compose logs app docreader postgres --since=10s -f
        else
