...
DOCS_ROOT = PROJECT_ROOT / "docs" / "archive" / "wnx"
ARCHITECTURE_DOC_PATH = PROJECT_ROOT / "docs" / "architecture" / "WEKNORA_NATIVE_EXPANSION_ARCHITECTURE.md"
...
REQUIRED_REPORTS = {
    "WNX-0-02": ARCHITECTURE_DOC_PATH,
...
def _run_phase5_report_checker_self_test() -> list[Issue]:
    result = subprocess.run(
        [
            sys.executable,
            str(PROJECT_ROOT / "scripts" / "validation" / "check_phase5_report_safety.py"),
            "--self-test",
        ],
    ...
    return [
        Issue(
            PROJECT_ROOT / "scripts" / "validation" / "check_phase5_report_safety.py",
