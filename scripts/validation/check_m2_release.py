...
STATIC_COMMANDS: tuple[tuple[str, list[str]], ...] = (
    ("compileall", [sys.executable, "-m", "compileall", "packages/knowledge-engine/knowledge_engine", "apps/pa-api/app", "packages/agent-runtime/agent"]),
    ("adapter error mapping", [sys.executable, "scripts/validation/smoke_weknora_adapter_errors_m2.py"]),
    ("RAG debug API", [sys.executable, "scripts/validation/smoke_rag_debug_api_m2.py"]),
    ("RAG debug params", [sys.executable, "scripts/validation/smoke_rag_debug_params_m2.py"]),
    ("citation contract", [sys.executable, "scripts/validation/smoke_weknora_citation_contract_m1.py"]),
    ("citation fail closed", [sys.executable, "scripts/validation/smoke_weknora_citation_fail_closed_m1.py"]),
    ("evidence dedup score", [sys.executable, "scripts/validation/smoke_evidence_dedup_score_m2.py"]),
    ("document recovery", [sys.executable, "scripts/validation/smoke_document_processing_recovery_m2.py"]),
    ("wiki status recovery", [sys.executable, "scripts/validation/smoke_wiki_status_recovery_m2.py"]),
    ("retrieve logging redaction", [sys.executable, "scripts/validation/smoke_weknora_logging_m2.py"]),
    ("request id propagation", [sys.executable, "scripts/validation/smoke_weknora_correlation_m2.py"]),
    ("history filters", [sys.executable, "scripts/validation/smoke_history_filters_m2.py"]),
    ("pilot feedback docs", [sys.executable, "scripts/validation/smoke_pilot_feedback_docs_m2.py"]),
    ("intranet runbook docs", [sys.executable, "scripts/validation/smoke_intranet_runbook_m2.py"]),
)

LIVE_COMMANDS: tuple[tuple[str, list[str], str], ...] = (
    (
        "M2 preflight: DeepSeek/DashScope/KB/vector/Redis/DocReader",
        [sys.executable, "scripts/validation/check_m2_preflight.py"],
        "live probes validate DashScope Embedding, KB embedding_model_id, vector dimension, Redis, DocReader",
    ),
    (
        "DeepSeek chat smoke",
        [sys.executable, "scripts/validation/smoke_real_chat_model_m2.py"],
        "calls configured real chat model through ModelGateway",
    ),
    (
        "WeKnora RAG live gate",
        [sys.executable, "scripts/validation/smoke_weknora_rag_m1.py"],
        "requires live WeKnora retrieval with non-mock evidence",
    ),
    (
        "WeKnora Wiki live gate",
        [sys.executable, "scripts/validation/smoke_weknora_wiki_m1.py"],
        "requires live WeKnora Wiki search/read/create path",
    ),
    (
        "Agent real LLM + non-mock citation",
        [sys.executable, "scripts/validation/smoke_weknora_agent_real_llm_m2.py"],
        "uploads one sanitized Markdown fixture and runs QA/policy/case through real LLM",
    ),
    (
        "Wiki real LLM draft + publish + retrieve",
        [sys.executable, "scripts/validation/smoke_wiki_real_llm_m2.py"],
        "uploads one sanitized fixture and publishes one generated PA Wiki page",
    ),
)

REQUIRED_FILES = (
    "scripts/validation/check_m2_preflight.py",
    "scripts/validation/smoke_real_chat_model_m2.py",
    "scripts/validation/smoke_weknora_rag_m1.py",
    "scripts/validation/smoke_weknora_wiki_m1.py",
    "scripts/validation/smoke_weknora_agent_real_llm_m2.py",
    "scripts/validation/smoke_wiki_real_llm_m2.py",
    "scripts/validation/smoke_weknora_logging_m2.py",
    "scripts/validation/smoke_weknora_correlation_m2.py",
...
def _check_release_docs() -> Gate:
    checklist = PROJECT_ROOT / "docs" / "archive" / "phase3" / "PHASE3_M2_RELEASE_CHECKLIST.md"
