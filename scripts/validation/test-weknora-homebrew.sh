...
#
# 用法:
#   ./scripts/validation/test-weknora-homebrew.sh                    # 完整测试（含前端构建）
#   SKIP_FRONTEND=1 ./scripts/validation/test-weknora-homebrew.sh    # 跳过前端构建
#   SKIP_BUILD=1 ./scripts/validation/test-weknora-homebrew.sh       # 跳过构建（使用已有 tarball）
#
...
if [ "${SKIP_BUILD:-}" != "1" ]; then
    ...
    SKIP_FRONTEND="${SKIP_FRONTEND:-0}" "$REPOSITORY_ROOT/scripts/release/package-weknora-lite.sh" "${VERSION}"
else
