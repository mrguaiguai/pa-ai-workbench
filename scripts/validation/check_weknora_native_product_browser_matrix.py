...
ROUTES = (
    {
        "name": "dialogue",
        "hash": "/",
        "markers": ("智能对话", "智能问答对话", "资料库", "检索", "问题"),
        "backend_marker": None,
    },
    {
        "name": "library",
        "hash": "/library",
        "markers": ("资料库", "知识库管理", "上传资料"),
        "backend_marker": "当前活动",
    },
    {
        "name": "analysis",
        "hash": "/analysis",
        "markers": ("功能已并入智能对话", "智能分析暂时冻结", "进入智能对话"),
        "backend_marker": "回答类型",
    },
    {
        "name": "rag-debug",
        "hash": "/rag-debug",
        "markers": ("检索问题", "原生知识问答", "暂无调试轨迹"),
        "backend_marker": "运行问答",
    },
    {
        "name": "wiki",
        "hash": "/wiki",
        "markers": ("搜索", "页面", "阅读", "高级维护"),
        "backend_marker": None,
    },
    {
        "name": "history",
...
def _validate_route(
    ...
    while time.time() < deadline:
        ...
        if all(marker in text for marker in route["markers"]):
            backend_marker = route.get("backend_marker")
            if backend_marker:
                _assert(backend_marker in text, f"{route['name']} route shows backend-backed state")
