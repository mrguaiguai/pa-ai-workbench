...
for arg in "$@"; do
  case "$arg" in
    ...
    -h|--help)
      cat <<'USAGE'
Usage: scripts/pa-workbench-start.sh [--build] [--skip-weknora] [--skip-pa] [--profile=NAME]

Start the local WeKnora stack and PA Workbench services.

This script does not print env values. Use scripts/pa-workbench-setup.sh first
...
"$ROOT_DIR/scripts/pa-workbench-setup.sh" --skip-install
...
if [[ "$START_PA" -eq 1 ]]; then
  ...
  "$PA_DIR/scripts/pa-dev-services.sh" start
fi

log "start command finished"
log "check: scripts/pa-workbench-check.sh"
