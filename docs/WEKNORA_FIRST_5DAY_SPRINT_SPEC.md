...
| WF-P2-03 | P2 | Vector store management visibility | [x] | Live PA API/browser shows sanitized native vector-store readiness, KB binding, and embedding readiness; CRUD/test/raw config stays backlog. |
...
| 2026-06-22 | WF-P2-03 | [x] | `live PA API/browser + native vector store read-only`: PA `/api/vector-stores/native/overview` reached native vector-store type catalog and store list with `status=live`, store types `7`, stores `1` (`env=1`, `user=0`), active KB binding `available/env/postgres`, embedding `live/openai_compatible/non-mock`, mutations `backlog`, and sanitized response true; homepage status strip rendered `Vector store` as live. | this commit on `weknora-first-mvp` | Report: `docs/WEKNORA_FIRST_VECTOR_STORE_VISIBILITY_REPORT.md`; vector-store CRUD, connection tests, raw config display, KB rebind mutation, and PA-owned vector administration remain explicit backlog. |
| 2026-06-22 | WF-P2-04 | [b] | `backlog + native source audit`: WeKnora graph/stats/lint/issues have safe read-only native routes already surfaced by `WF-P1-02`; remaining auto-fix, rebuild-links, and issue-status actions are owned-KB/admin mutations and require a separate safety design before PA exposure. | this commit on `weknora-first-mvp` | Report: `docs/WEKNORA_FIRST_WIKI_MAINTENANCE_BACKLOG_REPORT.md`; no mutation endpoint was called and no PA code was changed. |
