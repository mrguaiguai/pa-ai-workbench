...
SPEC_PATH = PROJECT_ROOT / "PHASE3_SPEC.md"
DOC_PATH = PROJECT_ROOT / "docs" / "PHASE3_STATIC_ACCEPTANCE.md"
...
STATIC_COMMANDS: tuple[tuple[str, list[str]], ...] = (
    (
        "M2 intranet runbook",
        [sys.executable, "backend/scripts/smoke_intranet_runbook_m2.py"],
    ),
    (
        "M2 pilot feedback docs",
        [sys.executable, "backend/scripts/smoke_pilot_feedback_docs_m2.py"],
    ),
    (
        "M3 local product static gates",
        [sys.executable, "backend/scripts/check_m3_local_product.py"],
    ),
)

REQUIRED_ARTIFACTS = (
    "backend/scripts/check_m1_release.py",
    "backend/scripts/check_m2_release.py",
    "backend/scripts/check_m3_local_product.py",
    "backend/scripts/check_phase3_static_acceptance.py",
...
def _check_m3_acceptance_coverage() -> Gate:
    text = SPEC_PATH.read_text(encoding="utf-8")
    runbook = (PROJECT_ROOT / "docs" / "PHASE3_M3_LOCAL_PRODUCT_RUNBOOK.md").read_text(encoding="utf-8")
    combined = f"{text}\n{runbook}".lower()
