"""Smoke-check policy workflow grounding with WeKnora evidence.

This fixture smoke covers P3-M1-D3:
- policy prompt requires numbered citations;
- run_analysis persists policy output with WeKnora document/wiki citations;
- no-evidence policy analysis returns explicit insufficient-evidence wording and
  a to-collect evidence list.
...
def _set_smoke_env() -> None:
    ...
    os.environ["WEKNORA_DEFAULT_KB_ID"] = "kb-policy-fixture"
...
class SmokeError(RuntimeError):
    """Raised when the policy prompt fixture contract fails."""
...
def main() -> int:
    try:
        ...
        print(f"WeKnora policy prompt smoke failed: {exc}", file=sys.stderr)
        return 1
    print("WeKnora policy prompt smoke passed (fixture)")
...
def _run_fixture_smoke() -> dict[str, Any]:
            ...
            "data": [
                {
                    "id": "chunk-policy-101",
                    "content": (
                        "Sanitized policy evidence states that external "
                        "briefings require owner, review date, and evidence list."
                    ),
                    "knowledge_id": "wk-policy-doc-101",
                    "knowledge_base_id": "kb-policy-fixture",
                    "knowledge_title": "Policy Fixture Document",
                    "chunk_index": 4,
                    "score": 0.048,
                    "match_type": 2,
                    "chunk_type": "text",
                ...
                },
                {
                    ...
                    "wiki_page_id": "wiki-policy-202",
                    "wiki_title": "Policy Fixture Wiki",
                    "content": (
                        "Sanitized wiki evidence says partner communication "
                        "risks must be reviewed before publication."
                    ),
                    "knowledge_base_id": "kb-policy-fixture",
                    "score": 0.066,
                    "metadata": {
                        "slug": "policy-fixture-wiki",
    ...
    for request in requests:
        ...
        if payload.get("knowledge_base_ids") != ["kb-policy-fixture"]:
...
def _assert_prompt_contract() -> None:
    request = AgentRequest(
        task_id="policy-prompt-unit",
        conversation_id="policy-prompt-conversation",
        task_type=AgentTaskType.POLICY_ANALYSIS,
        query_or_topic="Assess partner briefing policy changes.",
    )
    citation = AgentCitation(
        evidence_id="document_chunk:chunk-policy-101",
        source_type="document_chunk",
        external_doc_id="wk-policy-doc-101",
        chunk_id="chunk-policy-101",
        title="Policy Fixture Document",
        text="Sanitized policy prompt evidence.",
        score=0.048,
        source="weknora_api",
        metadata={},
    )
    prompt = PolicyAnalysisWorkflow._build_grounded_prompt(
        request,
    ...
    if missing:
        raise SmokeError("policy prompt missing fragments: " + ", ".join(missing))

    no_evidence_prompt = PolicyAnalysisWorkflow._build_grounded_prompt(
        request,
        AgentContext(request=request),
        [],
    )
    if "未检索到可用政策、案例或 Wiki 证据" not in no_evidence_prompt:
        raise SmokeError("policy prompt does not state no-evidence condition")
...
def _assert_run_analysis_contract() -> dict[str, Any]:
    ...
    with Session(engine) as session:
        _, _, evidence_task, evidence_output, evidence_citations = run_analysis(
            ...
            task_type=AgentTaskType.POLICY_ANALYSIS,
            query_or_topic="Assess partner briefing policy changes.",
            title="D3 Policy Evidence Smoke",
            business_area="public_affairs",
            extra_requirements="Use numbered citations for key judgments.",
        )
        ...
        required_markdown = [
            ...
            "[2]",
            "evidence_id=document_chunk:chunk-policy-101",
            "evidence_id=wiki_page:wiki-policy-202",
        ]
        missing_markdown = [
            fragment
        ...
        if missing_markdown:
            raise SmokeError("policy output missing fragments: " + ", ".join(missing_markdown))
        ...
        _, _, no_evidence_task, no_evidence_output, no_evidence_citations = run_analysis(
            session=session,
            task_type=AgentTaskType.POLICY_ANALYSIS,
            query_or_topic="no evidence policy topic",
            title="D3 Policy No Evidence Smoke",
            business_area="public_affairs",
            extra_requirements="Do not invent policy details.",
        )
        ...
        if "## 待补证据" not in no_evidence_markdown:
            raise SmokeError("no-evidence output missing to-collect evidence list")
        warnings = _json_array(no_evidence_output.warnings_json)
        if not any("No policy evidence" in warning for warning in warnings):
