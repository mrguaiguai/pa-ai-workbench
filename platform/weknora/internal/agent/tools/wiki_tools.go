...
func (t *wikiReadPageTool) Execute(ctx context.Context, args json.RawMessage) (*types.ToolResult, error) {
	...
	var outputs []string
	wikiPages := make([]map[string]interface{}, 0)
	var errs []string
...
%s
	...
	for _, slug := range slugsToFetch {
		...
		for _, sc := range effectiveScopes {
			...
			wikiPages = append(wikiPages, wikiPageReferenceData(page, actualKBID))
	...
	for _, query := range queriesToRun {
		...
		for _, sc := range effectiveScopes {
			...
			for _, p := range pages {
				...
				wikiPages = append(wikiPages, wikiPageReferenceData(p, actualKBID))
...
func wikiPageReferenceData(page *types.WikiPage, kbID string) map[string]interface{} {
	if page == nil {
		return nil
	}
	wikiPageID := page.ID
	if wikiPageID == "" {
		wikiPageID = page.Slug
	}
