...
LEGACY_WORKBENCH_DIR = REPO_ROOT / "pa-ai-workbench"
LEGACY_BACKEND_DIR = LEGACY_WORKBENCH_DIR / "backend"
LOCAL_RUNTIME_ROOT = REPO_ROOT / ".local"
CANONICAL_BACKEND_RUNTIME_DIR = LOCAL_RUNTIME_ROOT / "pa-api"


def _legacy_backend_runtime_present() -> bool:
    """Detect retained runtime state by path only; never inspect its contents."""

    return any(
        path.exists()
        for path in (
            LEGACY_BACKEND_DIR / ".env",
            LEGACY_BACKEND_DIR / ".venv",
            LEGACY_BACKEND_DIR / "data",
            LEGACY_BACKEND_DIR / "uploads",
        )
    )
...
_default_runtime_dir = (
    LEGACY_BACKEND_DIR
    if _legacy_backend_runtime_present()
    else CANONICAL_BACKEND_RUNTIME_DIR
)

# New checkouts use .local/pa-api. Existing private runtime state stays at its
# legacy path until the user explicitly migrates it, so this reorganization
# never silently switches databases, uploads, environments, or credentials.
# Validation can disable dotenv loading and therefore never reads local values.
if os.getenv("PA_SKIP_DOTENV", "").strip().lower() not in {"1", "true", "yes", "on"}:
    load_dotenv(REPO_ROOT / ".env")
    load_dotenv(LEGACY_WORKBENCH_DIR / ".env")
    load_dotenv(_default_runtime_dir / ".env")
    load_dotenv(WORKBENCH_DIR / ".env")
    load_dotenv(BACKEND_DIR / ".env")

BACKEND_RUNTIME_DIR = Path(
    os.getenv("PA_BACKEND_RUNTIME_DIR", str(_default_runtime_dir))
).expanduser()
