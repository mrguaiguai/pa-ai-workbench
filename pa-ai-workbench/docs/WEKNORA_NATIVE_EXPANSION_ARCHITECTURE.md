...
| --- | --- | --- | --- | --- |
| PA Frontend Shell | Navigation, pages, operator flow, status labels, citation/history display, safe jump links. | Native admin depth remains outside PA unless scoped. | `frontend/src/App.tsx`, `frontend/src/pages/*`, `frontend/src/api/client.ts`, `frontend/src/components/workbench.tsx` | `WNX-P0-03`, `WNX-P3-01`, P1 workflow pages. |
| PA Backend BFF | PA semantic APIs, status normalization, history/citation persistence, safe errors, masked config snapshots. | Native platform response shape and platform authorization. | `backend/app/api/*`, `backend/app/services/*`, `backend/app/schemas.py`, `backend/app/config.py` | `WNX-P0-02`, all P1/P2 BFF slices. |
| WeKnora Native Adapter | Thin typed client, trace id, timeout, retry, redaction, error classes, response normalization. | Native endpoints and authoritative capability behavior. | `knowledge_engine/backends/weknora_api_backend.py`, `knowledge_engine/errors.py`, `knowledge_engine/schemas.py`, `knowledge_engine/citations/*` | `WNX-P0-01` creates the shared client contract. |
| PA Business DB | Business records, history, citations, report metadata, active KB/workspace/config selection snapshots. | Chunks, vectors, provider secrets, parser internals, platform config truth. | `backend/app/models.py`, `backend/app/database.py`, SQLite by default | P1 history/citation and workflow persistence. |
| Evidence/Citation Layer | Traceable citation objects, locator routing, report safety, fail-closed checks. | Native ids and references returned by document, Wiki, or AgentQA paths. | `knowledge_engine/evidence.py`, `knowledge_engine/citations/*`, `backend/app/api/citations.py`, `backend/app/services/citation_locator_service.py` | `WNX-P1-07` plus each workflow task. |
| Validation/Ops Layer | Live smokes, browser checks, report safety, coverage ledger, service/runbook validation, handoff. | Native service health and platform status APIs. | `backend/scripts/*`, `docs/WEKNORA_FIRST_*`, future `docs/WEKNORA_NATIVE_CAPABILITY_COVERAGE_LEDGER.md` | `WNX-0-03`, `WNX-P0-04`, `WNX-P0-05`, `WNX-P3-02`. |
| PA-native Professional Workflow Layer | Policy/case/QA entry points, templates, PA history, report outputs, business language. | General AgentQA/custom Agent and tools when used for platform reasoning. | `agent/orchestrator.py`, `agent/agents/*`, `agent/tools/*`, `backend/app/services/analysis_service.py`, `frontend/src/pages/AnalysisPage.tsx` | P1 AgentQA/custom Agent integration, with deeper PA-native expansion backlog by default. |

## Request And Data Flow

### Document Lifecycle

1. PA Frontend Shell sends upload/status requests through `frontend/src/api/client.ts`.
2. PA Backend BFF receives requests in `backend/app/api/documents.py`.
3. `backend/app/services/document_service.py` creates or updates PA business
...
2. PA BFF routes through `backend/app/api/rag.py` and
...
2. PA BFF uses `backend/app/api/wiki.py` and `backend/app/services/wiki_service.py`.
...
- `frontend/src/App.tsx` owns hash navigation and the six current pages:
  Home, Library, Analysis, RAG Debug, Wiki, and History.
- `frontend/src/api/client.ts` owns frontend API types for status, model,
  documents, RAG, citations, Wiki overview, MCP overview, web search overview,
  and vector store overview.
- `frontend/src/pages/HomePage.tsx` owns status summary and entry points.
- `frontend/src/pages/LibraryPage.tsx` owns document upload, list, status,
  chunk/event viewing, and retry controls.
- `frontend/src/pages/RagDebugPage.tsx` owns native retrieval diagnostics and
  evidence display.
- `frontend/src/pages/AnalysisPage.tsx` owns PA professional analysis entry.
- `frontend/src/pages/WikiPage.tsx` owns Wiki browse/search/read/draft/status.
- `frontend/src/pages/HistoryPage.tsx` owns outputs, filters, warnings, and
...
- `backend/app/main.py` registers health, documents, citations, conversations,
  analysis, history, MCP, model, RAG, vector store, web search, and Wiki routers.
- `backend/app/api/health.py` exposes `/health`, `/api/status`, and
  `/api/capabilities`.
- `backend/app/api/documents.py` exposes PA document upload, list, status
  refresh, read, parse/index/reindex/retry, chunks, and events.
- `backend/app/api/rag.py` exposes `/api/rag/retrieve` and `/api/rag/debug`.
- `backend/app/api/wiki.py` exposes native Wiki overview plus PA Wiki
  search/page/draft/publish/status/reindex routes.
- `backend/app/api/analysis.py`, `backend/app/api/history.py`, and
  `backend/app/api/citations.py` preserve professional output, history, and
  locator workflows.
- `backend/app/api/mcp.py`, `backend/app/api/web_search.py`, and
  `backend/app/api/vector_store.py` expose native read-only overview slices.
- `backend/app/api/model.py` exposes model status.
...
- `knowledge_engine/backends/weknora_api_backend.py` centralizes much of the
...
Current PA DB models in `backend/app/models.py` include:
...
- Backend live smoke scripts under `backend/scripts/*` when a capability task
  changes behavior.
- Browser validation for affected frontend pages.
- Report safety checks such as `backend/scripts/check_phase5_report_safety.py`.
...
- `agent/orchestrator.py` registers PA workflows for knowledge QA, policy
  analysis, and case review.
- `agent/agents/*` owns the current PA-native workflow implementations.
- `agent/tools/*` owns retriever, Wiki reader/writer, citation checker, and
  evidence policy tools.
- `backend/app/services/analysis_service.py` persists PA analysis outputs and
  history.
- `frontend/src/pages/AnalysisPage.tsx` owns the operator-facing analysis UI.
...
| --- | --- | --- |
| Frontend shell/navigation | `frontend/src/App.tsx`, `frontend/src/styles.css` | Add capability/config center route in `WNX-P0-03`; keep six-page product flow. |
| Frontend API contracts | `frontend/src/api/client.ts` | Add status types only after BFF shape exists; do not infer health in UI. |
| Frontend pages | `frontend/src/pages/HomePage.tsx`, `LibraryPage.tsx`, `RagDebugPage.tsx`, `AnalysisPage.tsx`, `WikiPage.tsx`, `HistoryPage.tsx` | Browser validation required when touched. |
| BFF routers | `backend/app/api/*.py` | Expose PA semantic APIs and safe native overview endpoints. |
| BFF services | `backend/app/services/*.py` | Own business workflow, persistence, status, and citation glue. |
| Settings/config | `backend/app/config.py` | Load config but never print values; expose masked posture only. |
| DB models | `backend/app/models.py`, `backend/app/database.py` | Store business records, not WeKnora platform internals. |
| Native adapter | `knowledge_engine/backends/weknora_api_backend.py` | `WNX-P0-01` should split or standardize common request/response behavior before more capability work. |
| Citation contract | `knowledge_engine/evidence.py`, `knowledge_engine/citations/*`, `backend/app/api/citations.py`, `backend/app/services/citation_locator_service.py` | Fail closed on missing ids; status is not evidence. |
| Professional workflows | `agent/orchestrator.py`, `agent/agents/*`, `agent/tools/*` | Preserve PA workflow shell; avoid broad general Agent rebuild. |
| Validation | `backend/scripts/*`, `docs/WEKNORA_FIRST_*`, future WNX reports | Every task declares evidence type and runs focused checks. |
