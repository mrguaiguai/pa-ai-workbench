...
def _ensure_safe_mcp_server() -> subprocess.Popen[str] | None:
    ...
    server = subprocess.Popen(
        [
            sys.executable,
            str(PROJECT_ROOT / "backend" / "scripts" / "safe_local_mcp_server.py"),
            "--host",
            "0.0.0.0",
            "--port",
            str(SAFE_MCP_PORT),
...
def _validate_dialogue_viewport(
    *,
    ...
    try:
        ...
        _select_agent_scope(ws_url, agent_name=agent_name, kb_id=kb_id)
        ...
        _assert(scope.get("agent_selected") is True, f"{viewport['name']} selected scoped matrix Agent")
        _assert(scope.get("kb_scope") == kb_id, f"{viewport['name']} KB scope input is populated")
...
def _select_agent_scope(ws_url: str, *, agent_name: str, kb_id: str) -> None:
    script = f"""
    ...
    if (inputs[0]) setValue(inputs[0], 'WNID-P8-01 browser matrix');
    if (inputs[1]) setValue(inputs[1], {json.dumps(kb_id)});
    if (inputs[2]) setValue(inputs[2], '');
    Boolean(agent);
    """
    selected = _evaluate_cdp(ws_url, script)
    _assert(bool(selected), "browser selected the scoped WNID matrix Agent")
...
def _scope_metrics(ws_url: str) -> dict[str, Any]:
    value = _evaluate_cdp(
        ...
        """
        (() => {
          const active = document.querySelector('.dialogue-agent-option.active');
          const inputs = Array.from(document.querySelectorAll('.dialogue-control-grid input'));
          return {
            agent_selected: Boolean(active),
            kb_scope: inputs[1] ? inputs[1].value : '',
            knowledge_scope: inputs[2] ? inputs[2].value : ''
          };
        })()
        """,
    )
